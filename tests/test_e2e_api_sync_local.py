...
def _wait_for_request_completion(client: NeuroLinker, request_uid: str) -> dict:
    ...
    while time.time() < deadline:
        try:
            last = client.status.request(request_uid)
...
def test_e2e_local_pdfs_sync():
    ...
    with NeuroLinker.from_env() as client:
        ...
        extract_resp = client.extract.extract(documents=documents, urls=None, alias=None)
        ...
        res_images = client.documents.images(doc_ids)
