...
async def _wait_for_request_completion(client: AsyncNeuroLinker, request_uid: str) -> dict:
    ...
    while time.time() < deadline:
        try:
            last = await client.status.request(request_uid)
...
@pytest.mark.asyncio
async def test_e2e_local_pdfs_async():
    """
    Local upload E2E (async).
    """
    ...
    async with AsyncNeuroLinker.from_env() as client:
        ...
        extract_resp = await client.extract.extract(documents=documents, urls=None, alias=None)
        ...
        doc_status = await client.status.document(doc_ids[0])
        assert isinstance(doc_status, dict)
        assert "success" in doc_status

        res_json = await client.documents.json(doc_ids)
        ...
        res_sum = await client.documents.document_summary(doc_ids, summary_type="page")
        ...
        res_images = await client.documents.images(doc_ids)
