...
def test_embedding_base_functionality_sync() -> None:
    """Smoke: sync client reaches the embedding backend — a non-existent
    ...
    with NeuroLinker(token=TOKEN) as client:
        with pytest.raises(NeuroLinkerAPIError) as ei:
            client.embedding.jobs.get(FAKE_BUCKET_UID, FAKE_JOB_UID)

    assert ei.value.status_code == 404


@pytest.mark.asyncio
async def test_embedding_base_functionality_async() -> None:
    async with AsyncNeuroLinker(token=TOKEN) as client:
        with pytest.raises(NeuroLinkerAPIError) as ei:
            await client.embedding.jobs.get(FAKE_BUCKET_UID, FAKE_JOB_UID)
