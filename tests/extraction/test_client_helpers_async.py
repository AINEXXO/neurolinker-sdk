...
from neurolinker_sdk import AsyncNeuroLinker, extract_document_ids, extract_request_uid
...
async def test_async_client_uses_default_base_url_when_not_provided() -> None:
    ...
    async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as http_client:
        async with AsyncNeuroLinker(
            ...
            payload = await client.extraction.list_tasks()
...
async def test_wait_for_request_completion_async_handles_transient_404() -> None:
    ...
    async with AsyncNeuroLinker(
        ...
        result = await client.extraction.wait_for_request("req-async")
...
async def test_async_static_helper_methods_are_exposed() -> None:
    assert extract_request_uid({"request_uid": "req-x"}) == "req-x"
    assert extract_document_ids({"documents": [{"id": "doc-x"}]}) == ["doc-x"]
