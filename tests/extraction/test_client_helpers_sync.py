...
from neurolinker_sdk import NeuroLinker, extract_document_ids, extract_request_uid
...
def test_sync_client_uses_default_base_url_when_not_provided() -> None:
    ...
    with httpx.Client(transport=httpx.MockTransport(handler)) as http_client:
        with NeuroLinker(token="nl_dummy", http_client=http_client, timeout_s=1.0) as client:
            payload = client.extraction.list_tasks()
...
def test_extract_request_uid_accepts_top_level_or_data() -> None:
    assert extract_request_uid({"request_uid": "req-top"}) == "req-top"
    assert extract_request_uid({"data": {"request_uid": "req-data"}}) == "req-data"


def test_extract_request_uid_raises_on_missing_value() -> None:
    with pytest.raises(ValueError):
        extract_request_uid({"success": True})
...
def test_extract_document_ids_accepts_document_id_or_id() -> None:
    ...
    assert extract_document_ids(top) == ["doc-1", "doc-2"]
    assert extract_document_ids(nested) == ["doc-3", "doc-4"]
...
def test_wait_for_request_completion_sync_handles_transient_404() -> None:
    ...
    with NeuroLinker(
        ...
        client.extraction.status.request = fake_request

        result = client.extraction.wait_for_request("req-1")
